 ...
 */
package org.openscience.cdk1.qsar.descriptors.atomic;
...
import org.openscience.cdk1.Ring;
...
import org.openscience.cdk1.graph.invariant.ConjugatedPiSystemsDetector;
 ...
 * @cdk.bug     1632419
 */
@TestClass(value="org.openscience.cdk1.qsar.descriptors.atomic.RDFProtonDescriptor_GHR_topolTest")
	...
	if(atoms.size() > 0) {
		...
		for(double ghrt = limitInf; ghrt < limitSup; ghrt = ghrt + step) {  
			...
			for( int at = 0; at < atoms.size(); at++ ) {
				...
				mylist = org.openscience.cdk1.graph.BFSShortestPath.findPathBetween(mygraph,startVertex,endVertex);
